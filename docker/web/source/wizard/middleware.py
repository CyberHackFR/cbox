...
from .models import CBox, System, Network, WizardState
from source.extensions import db


class WizardMiddleware():
    """CyberBox Wizard Middleware."""
    ...
    def getMaxStep():
        ...
        """
        if CBox.query.order_by(CBox.id.asc()).count():
            # CBox exists, next step is smtp or verify
            return 'wizard.verify'
        if System.query.count():
            # Systems apart from CBox exist, next step is box4s
            return 'wizard.box4s'
